...
class Autentica extends Component 
{
    ...
    public function check_public($credentials) {
        ...
        // Check if the user exist
        $email = EmailPessoa_Front::findFirstByDescricao($credentials['email']);

        if ($email == false) {
//            $this->registerUserThrottling(0);
            throw new Exception('Dados de e-mail e senha errados! Tente novamente.');
        }
        $user = Usuario_Front::findFirstByIdEmailPessoa($email->id);

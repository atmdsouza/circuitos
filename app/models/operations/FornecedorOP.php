...
class FornecedorOP extends Fornecedor
{
    private $encode = "UTF-8";

    public function listar($dados)
    {
        return Fornecedor::pesquisarFornecedor($dados);
    }

    public function cadastrar(Fornecedor $objArray)
    {
    ...
    public function alterar(Fornecedor $objArray)
    {
        $manager = new TxManager();
        $transaction = $manager->get();
        try {
            $objeto = Fornecedor::findFirst($objArray->getId());
            $objeto->setIdCidadeDigital($objArray->getIdCidadeDigital());
            $objeto->setIdTipo($objArray->getIdTipo());
            $objeto->setDescricao(mb_strtoupper($objArray->getDescricao(), $this->encode));
            $objeto->setEndereco(mb_strtoupper($objArray->getEndereco(), $this->encode));
    ...
    public function ativar(Fornecedor $objArray)
    {
        $manager = new TxManager();
        $transaction = $manager->get();
        try {
    ...
    public function inativar(Fornecedor $objArray)
    {
        $manager = new TxManager();
        $transaction = $manager->get();
        try {
    ...
    public function excluir(Fornecedor $objArray)
    {
        $manager = new TxManager();
        $transaction = $manager->get();
        try {
    ...
    {
        try {
            $objeto = Fornecedor::findFirst("id={$id}");
            $objetoArray = array(
                'id' => $objeto->getId(),
